...
function Clock() {
    ...
    const start = () => {
        ...
        const timer = setInterval(()=>{
            setDate(new Date(restoreState('hw9-date', Date.now())))}, 1000)
        setTimerId(timer)
    ...
    const stringDay = new Intl.DateTimeFormat("en",{weekday: "long"}); // пишут студенты
    const stringMonth = new Intl.DateTimeFormat("en",{month: "long"}) || <br/> // пишут студенты
    ...
    return (
        <div className={s.clock}>
            ...
            <div className={s.buttonsContainer}>
                <SuperButton
                    id={'hw9-button-start'}
                    disabled={buttonDisabled} // пишут студенты // задизэйблить если таймер запущен
                    onClick={start}
                    className={!buttonDisabled ? s.button : s.buttonDisabled}
                >
                    Start
                </SuperButton>
                <SuperButton
                    id={'hw9-button-stop'}
                    disabled={!buttonDisabled} // пишут студенты // задизэйблить если таймер не запущен
                    onClick={stop}
                    className={buttonDisabled ? s.button : s.buttonDisabled}
                >
                    Stop
